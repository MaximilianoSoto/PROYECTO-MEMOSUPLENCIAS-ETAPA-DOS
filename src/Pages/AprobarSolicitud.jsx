...
const AprobarSolicitud = ({ data, setData, onClearData }) => {
  ...
  const filteredData = data.filter((item) => {
    ...
    const {
      nombreUnidad: searchProcedencia,
      date: searchFecha,
      nombreAusente: searchFuncionario,
      rutAusente: searchRutAusente,
    } = searchTerm;
  ...
  const handleSubmitForm = () => {
    ...
    if (newProgress[selectedRow] === 4) {
      ...
      const remainingData = newData.filter(
  ...
  */
  ...
  return (
    <div className=" w-full container pb-72 mx-auto h-screen">
      <div className=" my-2 p-5 border-2 border-black  rounded-md">
        ...
        <input
          ...
          onChange={(e) =>
            setSearchTerm({ ...searchTerm, nombreUnidad: e.target.value })
          }
        ...
        <input
          ...
          onChange={(e) =>
            setSearchTerm({ ...searchTerm, nombreAusente: e.target.value })
          }
          className="w-full py-2 px-4 border font-semibold border-gray-400 rounded mb-4"
        />
        <input
          ...
          onChange={(e) =>
            setSearchTerm({ ...searchTerm, rutAusente: e.target.value })
          }
          className="w-full py-2 px-4 border font-semibold border-gray-400 rounded mb-4"
        />
      </div>
        ...
        <tbody>
          {filteredData.map((item, index) => (
            <tr key={index} className="hover:bg-gray-100">
              <td className="py-2 px-4 border  border-1 border-gray-400">
                {item.id}
              </td>
              <td className="py-2 px-4 border  border-1 border-gray-400">
                {item.date}
              ...
              <td
                className={`py-2 px-4 border font-semibold text-2xl  border-1 border-gray-400 ${
              ...
              <td className="py-2 px-4 border  border-1 border-gray-400 text-center">
                <button
                  className="bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-4 rounded mb-2 mt-1"
                  onClick={() => handleApprove(item, index)}
                >
                  Aprobar
                </button>
                <button
                  className="bg-red-600 hover:bg-red-500 text-white font-bold py-2 px-3 rounded mb-2"
        ...
        <tbody>
          {aprobadas.map((item, index) => (
            <tr key={index} className="hover:bg-gray-100">
              ...
              <td className="py-2 px-4 text-center border  border-1 border-gray-400">
                <button
                  className="bg-blue-600 p-3 px-4 rounded-md font-semibold"
                  onClick={() => handleShowData(item)}
                >
                  {" "}
                  Ver{" "}
